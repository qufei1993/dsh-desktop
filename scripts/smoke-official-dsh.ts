...
const runtimeEnvironment = prependRuntimePath(process.env, [runtimeBin, path.dirname(node)])
...
try {
  ...
  console.log(`Official DSH smoke passed: pnpm 11.22.0 available, HTTP ${response.status}, bootstrap marker present`)
} finally {
  child.kill('SIGTERM')
